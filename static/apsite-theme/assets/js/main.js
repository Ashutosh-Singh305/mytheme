...
// for product category and product select by Ashu
document.querySelectorAll('form.contact-us-form').forEach(form => {
  const productCategorySelect = form.querySelector('.product-category');
  ...
  productCategorySelect.addEventListener('change', function () {
    ...
    if (!categoryId) {
      productSelect.innerHTML = '<option selected hidden>Select Interest In</option><option disabled>No options</option>';
    ...
    fetch(`/api/products-by-category/${categoryId}/`)
      ...
      .then(data => {
        ...
        if (Array.isArray(data.products) && data.products.length > 0) {
          ...
          productSelect.innerHTML = '<option selected disabled>No options</option>';
          productSelect.disabled = true;
        }
      })
      .catch(() => {
        productSelect.innerHTML = '<option selected disabled>Error loading options</option>';
        productSelect.disabled = true;
      });
  });
});
